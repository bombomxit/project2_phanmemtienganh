...
public class ControllerLevel4 implements Initializable {
    @FXML
    ...
    String tiengAnh;
    String linkMp3="";
    public void onBtnSubmit(ActionEvent event) {String w = txtBetWeen.getText();String word = first+w+end;
    ...
    public void onGoiy1(ActionEvent event) {
        ...
        if (!word.equals(tiengAnh)) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("gợi ý");
            alert.setHeaderText("Bạn  đã nhập sai " + word+"\nhãy nghe phát âm" +"\nphiên âm là "+spelling+"\ngợi ý là " +goiy );
            alert.show();
            String path = linkMp3;
    ...
    public void onGoiy3(ActionEvent event) { String w = txtBetWeen.getText();
    ...
    public void onGoiy4(ActionEvent event) {String w = txtBetWeen.getText();
        String word = first+w+end;
        if (!word.equals(tiengAnh)) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
